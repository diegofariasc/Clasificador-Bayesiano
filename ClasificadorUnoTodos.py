...
from numpy          import mean, std, median, shape, array, var
from scipy.io       import loadmat
from scipy.signal   import butter, lfilter
from scipy.stats    import norm, skew, kurtosis
from MuestraDatos   import MAD, ajustarNumeroExperimentos
...
Input:  datosSujetos =      Tupla en la forma ( S1 (canales,exp) , 
                            [S2(canales,exp),S3(canales,exp),..S9(canales,exp)] )
...
def removerOutliers ( datosSujetos, desplazamiento ):
    ...
    umbralCanalesA  = [ ( median(canal) - desplazamiento * MAD(canal),      # Calcular umbral entre el que se
                          median(canal) + desplazamiento * MAD(canal) )     # permitira pasar los datos
                      for canal in datosSujetos[0] ]
    n_canales, n_experimentos = shape ( datosSujetos[0] )                   # Calcular numero de canales y experimentos

    # Aplicar filtro
    caracteristicas_SA_Filtradas = filtrarOutliers (n_experimentos, n_canales, umbralCanalesA, datosSujetos[0].tolist( ) )
    ...
    for sujeto in datosSujetos[1]:

        # Calcular umbrales
        umbralCanalesB = [(median(canal) - desplazamiento * MAD(canal), median(canal) + desplazamiento * MAD(canal))
                          for canal in sujeto ]

        # Calcular numero de canales y experimentos
        n_canales, n_experimentos = shape( sujeto )

        #Crear lista con los datos filtrados
        caracteristicas_SB_Filtradas.append( filtrarOutliers (n_experimentos, n_canales, umbralCanalesB, sujeto.tolist( ) ) )
...
"""
def validacionCruzada ( datos, porcentajeEntrenar, canales, iteraciones, semilla ):
    ...
    for numIteracion in range ( iteraciones ):
        ...
        # Dividir datos de las clases B (e.g. S2, S3, ... S9)
        distribucionB = [ distribuirDatosEntrenamientoYClasificacion( sujeto, porcentajeEntrenar ) for sujeto in datos[1] ]
...
"""
def ejecutarClasificador( numeroSA, movimiento, freqInfFiltro, freqSupFiltro, Fs, funcionCaracteristicas, desplazamientoMAD,
                          porcentajeDatosEntrenar, canalesUtilizar, iteraciones, semilla ):
    ...
    caracteristicas = extraerCaracteristicas( datosFiltrados, funcionCaracteristicas, canalesUtilizar )
    print( "\nCaracteristica:", funcionCaracteristicas.__name__ )
    print("SujetoA:", shape( caracteristicas[0] ), " SujetosB:", [ shape( sujeto ) for sujeto in caracteristicas[1] ] )

    # Remocion de outliers
    print("\nDatos sin outliers (MAD)")
    datosSinOutliers = removerOutliers( caracteristicas, desplazamientoMAD )
    ...
    resultados = validacionCruzada( datosSinOutliers, porcentajeDatosEntrenar, canalesUtilizar, iteraciones, semilla )
...
def main():
    ...
    frecuenciaInferiorFiltro = 33               # Frecuencia inferior para filtro paso banda
    frecuenciaSuperiorFiltro = 46               # Frecuencia superior para filtro paso banda
    funcionCaracteristicas = std                # Funcion para la extraccion de caracteristicas

    ejecutarClasificador( sujetoAislado,
                          claseUtilizada,
                          frecuenciaInferiorFiltro,
                          frecuenciaSuperiorFiltro,
                          Fs,
